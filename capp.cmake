...
macro(capp_setup_config)
  if (NOT CAPP_CONFIGURATION)
    set(config_dir_regex "${CAPP_ROOT}/configuration/([^/])")
    if (CMAKE_CURRENT_SOURCE_DIR MATCHES "${config_dir_regex}")
      string(REGEX REPLACE "${config_dir_regex}" "\\1" config "${CMAKE_CURRENT_SOURCE_DIR}")
      set(CAPP_CONFIGURATION "${config}")
    endif()
  endif()
  if (NOT CAPP_CONFIGURATION)
    set(CAPP_CONFIGURATION "default")
  endif()
  set(CAPP_CONFIGURATION_ROOT "${CAPP_ROOT}/configuration/${CAPP_CONFIGURATION}")
  set(CAPP_BUILD_ROOT "${CAPP_CONFIGURATION_ROOT}/build")
  set(CAPP_INSTALL_ROOT "${CAPP_CONFIGURATION_ROOT}/install")
  set(config_file "${CAPP_CONFIGURATION_ROOT}/config.cmake")
  if (NOT EXISTS "${config_file}")
    message(FATAL_ERROR "configuration file ${config_file} doesn't exist, it should define any CMake variables needed to configure your build. If you have no such variables, creating an empty file will suffice.")
  endif()
  include("${config_file}")
...
function(capp_sentinel_file_valid)
  ...
  if (CAPP_CONFIGURATION_FILE)
    if (NOT "${sentinel_file}" IS_NEWER_THAN "${CAPP_CONFIGURATION_FILE}")
      set(${result_variable} FALSE PARENT_SCOPE)
      return()
    endif()
  endif()
...
function(capp_parse_main_args)
  ...
  while (remaining_args)
    ...
    elseif (arg MATCHES "${config_regex}")
      list(POP_FRONT remaining_args config)
    else()
  ...
  set(CAPP_CONFIGURATION "${config}" PARENT_SCOPE)
endfunction()
